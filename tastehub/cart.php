...
// Handle Quantity Updates or Item Deletion via POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_cart'])) {
        ...
        if ($quantity > 0) {
            $stmtUpd = $pdo->prepare("UPDATE cart SET quantity = ? WHERE cart_id = ? AND user_id = ?");
            $stmtUpd->execute([$quantity, $cartId, $userId]);
        } else {
            $stmtDel = $pdo->prepare("DELETE FROM cart WHERE cart_id = ? AND user_id = ?");
...
$stmtCart = $pdo->prepare("
    SELECT c.cart_id, c.quantity, f.food_id, f.food_name, f.price, f.discount_price, f.image 
    FROM cart c 
...
$deliveryFee = ($subtotal > 0) ? ($settings['delivery_fee'] ?? 5.00) : 0.00;
...
<head>
    ...
    <title>Shopping Cart - <?= e($settings['restaurant_name'] ?? 'Gourmet Bistro') ?></title>
...
<body>
    ...
    <section class="bg-dark text-white py-4 text-center">
        <div class="container">
            <h2 class="fw-bold mb-1"><i class="bi bi-cart3 me-2"></i>Your Shopping Cart</h2>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="card border-0 shadow-sm rounded-4 p-3 p-md-4">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center gap-3">
                                                        <img src="assets/images/<?= e($item['image']) ?>" class="rounded-3" width="60" height="60" style="object-fit: cover;" onerror="this.src='https://via.placeholder.com/60'">
                                                        <span class="fw-bold"><?= e($item['food_name']) ?></span>
                                                    </div>
                                                </td>
                                                ...
                                                <td>
                                                    <form action="cart.php" method="POST" class="d-flex align-items-center gap-1">
                                                        ...
                                                        <input type="number" name="quantity" value="<?= $item['quantity'] ?>" min="1" max="20" class="form-control form-control-sm text-center">
                                                        <button type="submit" name="update_cart" class="btn btn-sm btn-outline-secondary" title="Update"><i class="bi bi-check-lg"></i></button>
                                                    </form>
                                                </td>
                                                <td class="fw-bold text-danger">$<?= number_format($totalPrice, 2) ?></td>
                                                <td class="text-end">
                                                    <form action="cart.php" method="POST">
                                                        <input type="hidden" name="cart_id" value="<?= $item['cart_id'] ?>">
                                                        <button type="submit" name="remove_item" class="btn btn-sm btn-outline-danger border-0"><i class="bi bi-trash"></i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                                <a href="menu.php" class="btn btn-outline-secondary rounded-pill btn-sm px-3"><i class="bi bi-arrow-left me-1"></i> Continue Shopping</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card border-0 shadow-sm rounded-4 p-4">
                            ...
                            <a href="checkout.php" class="btn btn-primary-custom w-100 py-2 rounded-pill fw-medium">Proceed to Checkout <i class="bi bi-arrow-right ms-1"></i></a>
                        </div>
                    </div>
                </div>
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <a href="menu.php" class="btn btn-primary-custom rounded-pill px-4 mt-2">Browse Menu</a>
                </div>
            <?php endif; ?>
        </div>
    </section>
